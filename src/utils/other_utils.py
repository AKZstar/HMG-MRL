...
def scaffold_split_valid_test(df, frac=None, balanced=True, include_chirality=False, random_state=2020):
    ...
    Args:
        df (pd.DataFrame): 包含分子的原始数据。
        frac (list): 训练、验证、测试集的最终目标比例。
        balanced (bool): 是否使用平衡模式进行骨架划分。
        include_chirality (bool): 骨架是否包含手性信息。
        random_state_1 (int): 第一个随机数，用于固定测试集。
        random_state_2 (int): 第二个随机数，用于在训练/验证池中进行随机划分。
    ...
    print(f"最终验证集样本数: {len(final_valid)}")
